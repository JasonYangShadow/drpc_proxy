...
func (h *Handler) kafkaWorker() {
	for msg := range h.kafkaCh {
		...
		if len(b) > internal.KafkaMessageMaxSize {
			redisCtx, cancel := context.WithTimeout(context.Background(), internal.RedisTimeout)
			_ = h.store.SaveResponse(redisCtx, &internal.Response{
				Status:    "failed",
				RequestID: msg.RequestID,
		...
		err = h.producer.SendWithContext(kafkaCtx, msg.RequestID, b)
		kcancel()
...
func (h *Handler) HandleRPC(w http.ResponseWriter, r *http.Request) {
	...
	// Set initial pending status in Redis. Use Background context — this is
	// server-side state and must not be tied to the client connection lifetime.
	...
	default:
		...
		cleanCancel()
		metrics.ProxyQueueFullTotal.Inc()
		w.Header().Set("Content-Type", "application/json")
