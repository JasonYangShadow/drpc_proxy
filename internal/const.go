...
const (
	...
	KafkaConsumerJobChMultiplier   = 2                     // jobCh buffer = workers * batchSize * multiplier
	DefaultKafkaConsumerBatchSize  = 50                    // max messages per upstream batch request
	KafkaConsumerBatchFlushTimeout = 20 * time.Millisecond // flush accumulated batch after this idle window

	// upstream batch
	MaxUpstreamBatchResponseSize = 4 * 1024 * 1024 // 4MB max for a batch response body
