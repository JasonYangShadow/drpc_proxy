...
// Package load simulates N concurrent users each continuously sending
// fire-and-forget JSON-RPC requests to the DRPC proxy, with a random sleep
// between 0 and -gap between each request. Runs until Ctrl+C or -duration.
//
// Usage:
//
//	task test:load
//	task test:load USERS=50 GAP=200ms DURATION=5m
//
//	# or directly:
...
// Flags:
//
//	-users      number of simulated concurrent users (default 100)
//	-gap        max random sleep between requests per user (default 100ms)
...
var (
	fUsers    = flag.Int("users", 100, "number of simulated concurrent users")
	fGap      = flag.Duration("gap", 100*time.Millisecond, "max random gap between requests per user")
